        ...
        <table class="w-full">
          ...
          <tbody>
            <tr v-for="sale in sales" :key="sale.id" class="border-t hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-sm">
                <button @click="togglePaidStatus(sale)" class="text-primary-600 hover:text-primary-700 mr-2">
                  {{ sale.paid ? '↩️' : '✅' }}
                </button>
              </td>
            </tr>
        ...
        <div class="bg-white rounded-xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto">
          ...
          <form @submit.prevent="saveSale" class="space-y-4">
            ...
            <div class="flex items-center space-x-2">
              <input v-model="form.paid" type="checkbox" class="w-4 h-4" />
              <label class="text-sm font-medium text-gray-700">Pagamento realizado</label>
            </div>
            <div>
